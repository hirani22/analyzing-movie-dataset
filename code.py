...
def explore_data(dataset, start, end, rows_and_columns=False):
    ...
    extract_data = dataset[start:end]    
...
def duplicate_and_unique_movies(dataset, index_):
    ...
    print('Number of duplicate Movies:', len(repeated))
    print('\n')
    print('Examples of duplicate Movies:', repeated[:15])
...
for movie in movies:
    ...
    if (reviews_max[name] == no_reviews) and (name not in added_before):
        movies_clean.append(movie)
        added_before.append(name)
